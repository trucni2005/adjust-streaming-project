...
```
simulator (FastAPI) --> Postgres (schema adjust) --> Debezium (kconnect) --> Kafka cluster (3 broker) --> Spark Structured Streaming --> Iceberg (bronze_events)
```
...
5. Submit job Spark: `make spark-submit SPARK_LAYER=bronze SPARK_JOB=sink_to_bronze`.
